...
- **视角**：按住鼠标左键拖动可旋转视角（不强依赖 Pointer Lock）；也可点击画布捕获指针、按 `Esc`（或浏览器退出捕获）释放。
- **挖掘**：准星对准方块、**按住左键**持续命中，中央进度条满后天然方块被挖除，该材料入库存 +1；松开则进度重置。
- **放置**：`1-7` 热键或点击底部库存栏切换选中材料；**右键**把选中材料放到准星相邻的合法空格（库存 −1，放置后方块标记为「玩家放置」）。
- **拆除**：对玩家放置的方块按住左键（与挖掘同机制），完成后全额返还材料。
- **库存**：底部栏显示 7 种材料数量与当前选中高亮；与 `window.__app.state.inventory/selected` 同源。
