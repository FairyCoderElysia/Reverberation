...
/** 方向性校验的必需材料名（按 name 查找，不绑定数组下标，脆弱性修复 Code-m4） */
export function tableRowsByName(rows: readonly MaterialRow[]): Partial<Record<MaterialName, MaterialRow>> {
  const out: Partial<Record<MaterialName, MaterialRow>> = {};
 ...
 * 按材料 name 查找（而非固定数组下标）；返回违规描述列表，空数组 = 全部通过。
 */
export function validateTable(rows: readonly MaterialRow[]): string[] {
  const errors: string[] = [];
  const byName = tableRowsByName(rows);
  const foam = byName.foam;
  const glass = byName.glass;
  const stone = byName.stone;
  const concrete = byName.concrete;
  const metal = byName.metal;
  const soil = byName.soil;

  if (!foam || !glass || !stone || !concrete || !metal || !soil) {
    errors.push('0 材料表缺少方向性校验所需材料（foam/glass/stone/concrete/metal/soil）');
    return errors;
  }
...
/** 合并覆盖值并夹取到合法域（Code-M1）：abs/trans → [0,1]、mass → ≥0、durability → 正整数；非有限数忽略（保留默认）。 */
export function applyOverride(
  ...
  if (override.abs) {
    for (const b of [0, 1, 2] as const) {
      ...
      if (v !== undefined && Number.isFinite(v)) abs[b] = clampUnit01(v);
    }
  }
  if (override.trans) {
    for (const b of [0, 1, 2] as const) {
      const v = override.trans[b];
      if (v !== undefined && Number.isFinite(v)) trans[b] = clampUnit01(v);
    }
  }
  let mass = row.mass;
  if (override.mass !== undefined && Number.isFinite(override.mass)) {
    mass = Math.max(0, override.mass);
  }
  let durability = row.durability;
  if (override.durability !== undefined && Number.isFinite(override.durability)) {
    durability = Math.max(1, Math.floor(override.durability));
  }
  return { name: row.name, mass, durability, abs, trans };
}

/** 计算当前有效材料行（MaterialRow[]，供方向性校验等复用，与 effectiveMaterials 同一来源） */
export function effectiveRows(overrides: ReadonlyMap<number, MaterialOverrides>): MaterialRow[] {
  return MATERIAL_TABLE.map((row, id) => applyOverride(row, overrides.get(id) ?? {}));
}
...
export function effectiveMaterials(overrides: ReadonlyMap<number, MaterialOverrides>): MaterialSpec[] {
  return effectiveRows(overrides).map((row, id) => buildMaterialSpec(id, row));
}
